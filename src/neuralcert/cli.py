"""Single command-line entrypoint for every bundled NeuralCert problem."""
...
HELP = """usage: neuralcert COMMAND [options]
...
Run `neuralcert COMMAND --help` or `neuralcert sign|delsarte COMMAND --help`
...
def _delsarte(args: list[str]):
    if not args or args[0] in {"-h", "--help"}:
        print("usage: neuralcert delsarte {bound,hierarchy,verify} [options]")
        return 0
    stage, rest = args[0], args[1:]
    if stage == "bound":
        from neuralcert.problems.delsarte.cli import main
        return main(rest)
    if stage == "hierarchy":
        from neuralcert.problems.delsarte.hierarchy_cli import main
        return main(rest)
    if stage == "verify":
        from neuralcert.problems.delsarte.verifier import main
        return main(["neuralcert", *rest])
    raise SystemExit(f"unknown Delsarte workflow {stage!r}")


def _sign(args: list[str]):
    if not args or args[0] in {"-h", "--help"}:
        print("usage: neuralcert sign {discover,collocate,laguerre,hybrid,certify,verify} [options]")
        return 0
    stage, rest = args[0], args[1:]
    if stage == "verify":
        from neuralcert.problems.sign_uncertainty.verifier import main
        return main(rest)
    modules = {
    ...
    module = __import__(
        f"neuralcert.problems.sign_uncertainty.{module_name}",
        fromlist=["main"],
    )
...
def main(argv: Sequence[str] | None = None):
    ...
    if command in specialized:
        ...
        return _legacy_main(module.main, rest)
    raise SystemExit(f"unknown NeuralCert command {command!r}; run `neuralcert --help`")
