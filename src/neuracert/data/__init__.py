"""Bundled reference datasets for reproducible NeuraCert experiments."""
...
def dataset(name: str) -> Traversable:
    ...
    exact CSV filename. The returned resource also works when NeuraCert is
    ...
    if filename not in DATASETS.values():
        ...
        raise KeyError(f"unknown NeuraCert dataset {name!r}; available: {available}")
