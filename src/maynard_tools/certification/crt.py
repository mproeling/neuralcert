...
problem M_{k,eps,1/2}, with c-weighted channel pruning.  v3 backend for the
...
def build_payloads(channels: list[ChannelData], c_q, k: int,
                   eps: Fraction) -> tuple[GlobalDims, list[PairPayload]]:
    if k < 2:
...
class CRTAggregator:
    ...
    def run(self, jobs: int = 1, chunk: int = 4, verbose: bool = True,
        ...
        if resume_path:
            import hashlib
            import os
            key = repr((self.dims, sorted((pl.j, pl.l, pl.bh, pl.tw, pl.cc)
...
def certify_epsilon_crt(npz_path: str, degrees: list[int], bits: int,
    ...
    for d in degrees:
        ...
        # FRAME MAP: _ritz whitens A internally, but the A_f it received is the
        # per-channel diagonally PRECONDITIONED Gram (diag ~ 1), so the c it
        # returns lives in the preconditioned frame c_hat = D^{1/2} c_true.
        # The exact aggregator evaluates c on the TRUE channels, so c_float
        # must be mapped back: c_true = exp(-logA_diag/2) c_hat.  Omitting
        # this certifies the preconditioned c as if true-frame and yields a
        # value far below the float prediction (k=53: float 3.99 -> exact
        # 3.59).  (The use_nn_c/no_prune branch sets c_float from the loader,
        # which is already true-frame, so guard on having run the float pass.)
        if not (use_nn_c and no_prune):
            active_arr = np.asarray(active, dtype=int)
            c_float = c_float * np.exp(-0.5 * logA_diag_f[active_arr])
        ...
        if not (use_nn_c and no_prune):
            # Frozen-c gate: the float prediction must survive freezing.
            # c_float is now TRUE-frame; A_f/B_f are preconditioned, so map the
            # frozen c into the preconditioned frame (c_hat = D^{1/2} c_true)
            # for a consistent quadratic form.
            cq_np = np.array([float(f) for f in c_q])
            half_act = 0.5 * logA_diag_f[np.asarray(active, dtype=int)]
            ch_hat = np.exp(half_act) * cq_np
        ...
        print(f"  exact pass: {len(agg.payloads)} pairs, "
              ...
              f" (+{agg.verify_primes} verify)")
        rp = (f"{resume}.d{d}" if resume else None)
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="Multimodular exact certification of the epsilon-enlarged "
